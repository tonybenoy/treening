    ...
    html! {
        <div class="px-4 py-4 transition-colors duration-200">
            ...
            <div class="flex flex-wrap gap-2 mb-6">
                ...
                <span class="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-transparent rounded text-xs font-bold uppercase tracking-wider">{ex.equipment.to_string()}</span>
                { if ex.is_custom {
                ...
                html! {
                    <div class="mb-6 flex justify-center">
                        <img
                            src={img.clone()}
                            alt={ex.name.clone()}
                            class="w-full max-w-sm rounded-xl bg-gray-50 dark:bg-gray-800 p-2 border border-gray-200 dark:border-gray-700 shadow-sm transition-colors"
                        />
                    </div>
                }
            } else {
            ...
            <div class="mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    { for ex.muscle_groups.iter().map(|m| html! {
                        <span class="px-3 py-1 bg-gray-100 dark:bg-gray-800 rounded-lg text-sm text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-transparent transition-colors">{m}</span>
                    })}
                </div>
            </div>
            <div class="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-4 border border-gray-200 dark:border-gray-700/50 transition-colors">
